...
import { TREATMENTS, TREATMENTS_SECTION } from '../data';
    ...
    <div id="treatments" className="space-y-6 scroll-mt-24">
      ...
      <div className="space-y-4">
        {TREATMENTS.map((t, index) => {
          const IconComponent =
            (Icons as Record<string, React.ComponentType<{ size?: number }>>)[t.iconName] ||
            Icons.Activity;
              ...
              <dl className="mt-4 space-y-3 sm:pl-14">
                {DETAIL_ROWS.map(({ label, key }) => (
                  <div key={key} className="text-sm leading-relaxed">
                    ...
                    <dd className="font-sans text-stone-600">{t[key]}</dd>
                  </div>
                ))}
              </dl>
